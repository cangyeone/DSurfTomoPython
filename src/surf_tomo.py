...
def depth_sensitivity(period: float, depz: np.ndarray, surface_speed: float, minthk: float) -> np.ndarray:
    """Approximate depth-dependent surface-wave sensitivity.

    The Fortran workflow perturbs each layer and recomputes dispersion curves to
    build partial derivatives.  Here we approximate that behavior with an
    exponential decay that honors the period-dependent penetration depth and the
    layer thickness (``minthk`` acts as a floor on the decay scale).  The weights
    are normalized so they sum to one across all layers.
    """
    ...
    penetration = max(minthk, 0.25 * surface_speed * period)
    mids = 0.5 * (depz[:-1] + depz[1:])
    weights = np.exp(-mids / penetration) * (depz[1:] - depz[:-1])
    total = weights.sum()
    if total <= 0:
...
def build_system(
    ...
    for s_idx, src in enumerate(sources):
        ...
        for rcv in src.receivers:
            ...
            layer_weights = depth_sensitivity(src.period, model.depz, model.vsf[i_src, j_src, 0], minthk)
            for (i, j), seg in zip(path.cells, path.lengths):
                if 0 < i < grid.nx - 1 and 0 < j < grid.ny - 1:
                    for k, depth_w in enumerate(layer_weights):
                        idx = cell_index(i, j, k)
                        slowness = 1.0 / model.vsf[i, j, k]
                        dsyn[row] += seg * depth_w * slowness
                        rows.append(row)
                        cols.append(idx)
                        data.append(-seg * depth_w * slowness ** 2)
            row += 1
...
def run_iteration(
    ...
    A, b, dsyn = build_system(model, grid, sources, obs, weight, minthk)
...
def driver(input_path: Path) -> None:
    ...
    for it in range(1, cfg.maxiter + 1):
        dv, dsyn = run_iteration(model, cfg.grid, sources, obs, cfg.weight0, cfg.threshold0, cfg.minthk)
